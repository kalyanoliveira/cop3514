...
class Dog
{
    ...
    public:
        ...
        Dog(string new_name, string new_owner_name, int new_num_nights)
        {
            name = new_name;
...
void calculate_cost(Dog list[], int nd);
...
int main() {
    ...
    for (;;) 
    {
        cout << "Enter operation code" << std::endl;
        cout << "i for insert" << std::endl;
        cout << "s for search" << std::endl;
        cout << "p for print" << std::endl;
        cout << "c for cost" << std::endl;
        cout << "q for quit" << std::endl;
        cin >> code;

        // Clears the rest of stdin.
...
// Calculates the stay cost for a dog.
void calculate_cost(Dog list[], int nd)
{
    string dog_name, owner_name;
    cout << "Enter dog name: ";
    ...
    int i = find_dog(dog_name, owner_name, list, nd);
    if (i >= 0) {
        cout << "Total cost for dog is $" << list[i].get_num_nights() * RATE << endl;
    } else {
        cout << "Dog not found." << std::endl;
    }
}
